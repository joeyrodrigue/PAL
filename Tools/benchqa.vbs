...
Set MySink1 = WScript.CreateObject("WbemScripting.SWbemSink","SINK1_")
...
objWMIservice.ExecNotificationQueryAsync MySink1,"SELECT * FROM __InstanceModificationEvent WITHIN 3 WHERE TargetInstance ISA 'Win32_PerfFormattedData_PerfProc_Process' and (targetinstance.name like 'BESClient%' or targetinstance.name like 'Presell%' or targetinstance.name like 'Ranorex%')"
objWMIservice.ExecNotificationQueryAsync MySink2,"SELECT * FROM __InstanceModificationEvent WITHIN 3 WHERE TargetInstance ISA 'Win32_PerfFormattedData_PerfProc_Process' and (targetinstance.Name='_Total' or targetinstance.name like 'Po%' or targetinstance.name like 'Pe%' or targetinstance.name like 'MicroHost%' or targetinstance.name like 'source%' or targetinstance.name like 'winlogbeat%' or targetinstance.name like 'ir_agent%' or targetinstance.name like 'gs.%' or targetinstance.name like 'NotificationBar%' or targetinstance.name like 'GameStop%' or targetinstance.name like 'metric%' or targetinstance.name like 'New%' or targetinstance.name like 'Cy%')"
...
SUB SINK1_OnObjectReady(objObject,objAsyncContext)
