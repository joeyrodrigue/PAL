...
namespace Alpha
{
    ...
    public class fnDoScenario19 : ITestModule
    {
        /// <summary>
        /// Constructs a new instance.
        /// </summary>
        public fnDoScenario19()
        {
        ...
        //*****************Start  Scenario 19 - Cover Art ******************
			Global.CurrentScenario = 19;
	...
	//			if(Global.CurrentScenario != 9999)
			...
			Global.LogText = @"---> fnDoScenario19 Iteration: " + Global.CurrentIteration;
			WriteToLogFile.Run();	        	
            Report.Log(ReportLevel.Info, "Scenario 19 IN", "Iteration: " + Global.CurrentIteration, new RecordItemIndex(0));			
	        ...
	        Global.Module = "Cover Art:";                
	        DumpStatsQ4.Run();  			
			
	        MystopwatchQ4.Reset();	    
			MystopwatchQ4.Start();	            
            
			Global.LogText = @"Clicking on F2 Cover Artl";
			WriteToLogFile.Run();                
            repo.BackOffice275111HomeScreen.BackOffice275111HomeScreen.PressKeys("{F2}");
            Thread.Sleep(100);
			...
			TimeMinusOverhead.Run((float) MystopwatchQ4.ElapsedMilliseconds);  // Subtract overhead and store in Global.Q4StatLine            
	        Global.CurrentMetricDesciption = "[F2] Cover Art and login";
	        Global.Module = "Cover Art:";                
	        DumpStatsQ4.Run();  			
			...
			Global.LogText = @"Waiting for Cover Art screen"; 
			WriteToLogFile.Run(); 			
           	while(!Host.Local.TryFindSingle(repo.BaseArtWorkForm.LblTitleInfo.AbsolutePath.ToString(), out element))	
            {	
            	Thread.Sleep(100);
            }            

			TimeMinusOverhead.Run((float) MystopwatchQ4.ElapsedMilliseconds);  // Subtract overhead and store in Global.Q4StatLine            
	        Global.CurrentMetricDesciption = "display Cover Art listing";
	        Global.Module = "Cover Art:";                
	        DumpStatsQ4.Run();  			
            ...
            Global.CurrentMetricDesciption = @"Scenario 19";
			...
			Global.LogText = "<--- fnDoScenario19 Iteration: " + Global.CurrentIteration;
			WriteToLogFile.Run();	      			
            Report.Log(ReportLevel.Info, "Scenario 19 OUT", "Iteration: " + Global.CurrentIteration, new RecordItemIndex(0));	
